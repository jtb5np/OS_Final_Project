	...
	if (true) {
		match x { 
			...
			92		=>	{
				...
				else if (tbar.menu_open && cursor_in(0, 428, 100, 20))
				{
					win_count += 1;
...
impl taskbar {
	...
	pub unsafe fn clear_menu(&mut self) {
		io::fill_box(0, 428, 100, 20, io::BG_COLOR);
	}

	pub unsafe fn draw(&mut self) {
		let menu = cstr::from_str(&"Menu");
		io::fill_box(1, 450, 635, 27, 0x666666);
		io::draw_box(0, 449, 637, 29, 0x000001);
		io::draw_line(57, 450, 57, 478, 0x000001);
		if (self.menu_open) {
			let open_win = cstr::from_str(&"Open Window");
			io::fill_box(0, 428, 100, 20, 0x888888);
			io::draw_box(0, 428, 100, 20, 0x000000);
			drawcstr_at_coord(open_win, 6, 430, 0x000000);
		}
		drawcstr_at_coord(menu, 11, 456, 0x000001);
		menu.destroy();
	}
}
